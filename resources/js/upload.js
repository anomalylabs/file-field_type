...
$(function () {
    ...
    var dropzone = new Dropzone('.dropzone:not(data-initialized)',
        {
            paramName: 'upload',
            url: REQUEST_ROOT_PATH + '/admin/file-field_type/handle/' + key,
            headers: {
            ...
            accept: function (file, done) {
                $.post(REQUEST_ROOT_PATH + '/admin/file-field_type/exists/' + element.data('folder') + '/' + key, {'file': file.name}, function (data) {
                    if (data.exists) {
    ...
    dropzone.on('queuecomplete', function () {
        ...
        uploader.find('.uploaded').load(REQUEST_ROOT_PATH + '/admin/file-field_type/recent/' + key + '?uploaded=' + uploaded.join(','));
    });
});
